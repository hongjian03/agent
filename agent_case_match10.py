...
class PromptTemplates:
    def __init__(self):
        self._templates = {
            ...
            1. 国家标签分析
            ● 仅将客户明确签约的国家作为国家标签，例如：美国、英国、加拿大等，无需添加'申请'字样；
            ...
            ● "顶级名校成功案例"标签判断：
            ○ 必须满足以下条件之一：
            ■ 条件路径一：申请目标院校在下方顶级名校列表中
            ■ 条件路径二：同时满足以下全部要求
                □ 背景特别优秀（必须同时满足以下两个条件）
                ▪ 条件1：优质院校定义（必须符合以下之一）：
                    - 中国大陆院校：985/中外合作办学院校
                    - 非美国地区国外院校：全球TOP50
                    - 美国地区院校：U.S. News排名前30
                ▪ 条件2：学术表现要求：
                    - 中国大陆标准：GPA ≥ 87分或3.7/4.0
                    - 英国中外合作办学：GPA >= 70分(即一等学位)
                    - 其他国家中外合作办学：按照该国家的评分标准判断是否达到优秀学术表现
                □ 明确表达了对列表中顶级名校的申请意向
            ○ 【重要提醒】：
            ■ 输出此标签时必须使用具体国家名称替换"XX"，例如"美国顶级名校成功案例"、"英国顶级名校成功案例"等
            ■ 不得直接输出"XX国家顶级名校成功案例"
            ■ 仅能使用列表中包含的国家/地区名称
            ○ 顶级名校列表：
            ...
            ○ 【第一步】确定案例类型（必须先完成这一步）：
            ■ 博士申请案例：留学/升学类别为"博士"或"研究型硕士"
            ■ 低龄留学案例：留学/升学类别为"K12"或相关中小学申请
            ■ 顶级名校案例：满足第3部分"XX国家顶级名校成功案例"的条件
            ...
            ■ 【重要】对于以下四类案例，严禁输出"熟练Lv.1+"，只能输出"资深Lv.3+"和"专家Lv.6+"：
                □ 博士申请案例
                □ 低龄留学案例
                □ 顶级名校案例
                □ 高关注案例（包括家长密集关注、强沟通需求、频繁解释确认、或客户来自特定院校背景）   
            ■ 对于上述四类案例，严格检查，确认输出结果中不包含"熟练Lv.1+"标签
            ■ 如果满足以下任一情况，仅输出专家(Lv.6+)：
                □ 客户或家长表现出极高难度的沟通需求
                □ 案例整体难度和复杂度特别高
                □ 学生教育背景极好（例如毕业于美国U.S. News排名前30的美国院校，英国QS排名前50的院校等、GPA很高、有多段科研经历）
            ■ 只有对于常规申请案例（不属于上述任何特殊类型），才同时输出所有三个标签："熟练Lv.1+"、"资深Lv.3+"和"专家Lv.6+"

            ○ 【最终检查】（必须执行）：
            ■ 如果案例有以下任一特征，检查并确认输出中没有"熟练Lv.1+"标签：
                □ 涉及博士申请
                □ 涉及低龄留学
                □ 涉及顶级名校申请
            ...
            ● 顾问背景标签：
            ○ 海归：【严格限制】仅在客户明确表达"需要具有海外留学/工作经历的顾问"或类似直接要求时才输出此标签。客户或学生自身的学校名称、背景描述中出现"海归"等词汇不构成对顾问背景的要求。判断时必须基于客户对顾问的明确要求，而非对案例其他信息的推断。

            ● 地域标签：
            ○ 只有在客户对地域有要求的情况下，需要根据客户所在地及具体需求，输出相应的地域标签
