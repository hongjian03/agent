    ...
    def __init__(self):
        self._templates = {
            ...
            2. 掌握六大维度标签体系：
                ...
                - 院校层次（根据备注信息中提及海归情况输出）
                - 业务单位所在地（根据备注信息中对文案所在地要求输出）
            3. 精准进行学生信息到标签的映射
            4. 确保标签选择的优先级合理
            """,
            ...
            5. 院校层次标签
            - 根据special_notes中提及的海归情况输出院校层次标签
            - 如果没有相关信息则不输出
            
            6. 业务单位所在地标签
            - 根据special_notes中对文案所在地的要求输出业务单位所在地标签
            - 如果没有相关要求则不输出
            """
        }
...
def extract_tags_task(step_callback, current_prompt=None):
    ...
    {{
      "recommended_tags": {{
        ...
        "businessLocation": ["string, 业务单位所在地"]
      }}
    }}
    """
            ...
            try:
                ...
                if isinstance(recommended_tags, dict):
                    ...
                    for category in ["countries", "majors", "businessCapabilities", 
                                   "serviceQualities", "stability", "schoolLevel", "businessLocation"]:
