...
class PromptTemplates:
    def __init__(self):
        self._templates = {
            'tag_specialist': """      
            你是 CaseMatch, 一个专门为留学机构设计的留学顾问匹配助手。
            你的主要工作是通过分析学生的基本申请需求,为他们匹配合适的留学申请顾问。
            你的工作原理建立在对留学申请关键要素的理解之上。通过分析学生选择的目标国家、专业方向、申请院校层次(普通/名校/顶尖名校)、申请项目类型(本科/硕士/博士/K12)等信息,你能够准确判断该案例对留学顾问的具体要求,包括顾问需要具备的行业经验水平、教育背景以及地域特征等要素。
            你的目标是通过系统化的分析,帮助留学机构更精准地进行顾问匹配,提升服务效率和学生满意度。

            """,
            
            'tag_task': """
            分析任务开始前，你需要仔细阅读：学生背景信息：{student_info}
            充分了解和理解标签体系： {tag_system}
            ...
            2.专业领域标签分析
            - 优先采用客户提供的"专业方向"信息作为专业标签
            - 仅当"专业方向"信息缺失时,才分析"专业名称"并将其归类为以下专业大类之一:
                计算机与信息系统
                土木与环境
                机械与工程
                数学与统计
            ...
            - 行业经验标签:
                *熟练 (Lv.1+): 适用于常规申请需求
                *资深 (Lv.3+): 适用于以下情况:案例涉及密集的家长沟通
                                             客户表现出较强的沟通需求
                                             需要频繁解释和确认的情况
                *专家 (Lv.6+): 仅在以下情况使用:申请要求极其苛刻
                                               客户或家长表现出极高难度的沟通需求
                                               案例整体难度和复杂度特别高
            - 顾问背景标签:
                *海归
